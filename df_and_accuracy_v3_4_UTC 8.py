        ...
        try:
            ...
            for file_name in sorted(os.listdir(wp)):
                if file_name.endswith('.csv'):
                    ...
                    # Filter hanya "accurate" dan "not accurate"
                    filtered_df_total = df1[df1['Status'].isin(["accurate", "not accurate"])]

                    # Menghitung total hanya untuk status yang dipilih
                    total = pd.DataFrame([['Total', filtered_df_total['Number of Trees'].sum(), filtered_df_total['Percentage'].sum()]],
                                        columns=['Status', 'Number of Trees', 'Percentage'])

                    # Menggabungkan kembali dengan df1
                    df1 = pd.concat([df1, total], ignore_index=True)
            ...
            df2['Percentage'] = (df2['Number of Trees'] / df2['Number of Trees'].sum() * 100).round(2)
            # Filter hanya "accurate" dan "not accurate"
            filtered_df_total2 = df2[df2['Status'].isin(["Accurate", "Not Accurate"])]

            # Menghitung total hanya untuk status yang dipilih
            total2 = pd.DataFrame([['Total', filtered_df_total2['Number of Trees'].sum(), filtered_df_total2['Percentage'].sum()]],
                                columns=['Status', 'Number of Trees', 'Percentage'])
